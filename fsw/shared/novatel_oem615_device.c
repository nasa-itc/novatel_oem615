...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf(
            "NOVATEL_OEM615_CommandDevice - uart_flush returned error with status = %d! No uart_write_port performed.",
            status);
#endif
        status = OS_ERROR;
    }
                                           ...
                                           int8_t period_option)
{
    ...
    if (status == OS_SUCCESS)
    {
        ...
        {
#ifdef NOVATEL_OEM615_CFG_DEBUG
            OS_printf("NOVATEL_OEM615_CommandDeviceCustom - uart_flush returned error with status = %d! No "
                      "uart_write_port performed.",
                      status);
#endif
            status = OS_ERROR;
        }
...
int32_t NOVATEL_OEM615_RequestHK(uart_info_t *uart_device, NOVATEL_OEM615_Device_HK_tlm_t *data)
{
    ...
    if (status == OS_SUCCESS)
    {
        ...
        if (status == OS_SUCCESS)
        {
            ...
            if ((read_data[0] == NOVATEL_OEM615_DEVICE_HDR_0) && (read_data[1] == NOVATEL_OEM615_DEVICE_HDR_1) &&
            ...
            {
#ifdef NOVATEL_OEM615_CFG_DEBUG
                OS_printf("  NOVATEL_OEM615_RequestHK: NOVATEL_OEM615_ReadHK returned data with either invalid header "
                          "[0x%02x%02x] or invalid trailer [0x%02x%02x]!\n",
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf(
            "  NOVATEL_OEM615_ReadHK: Structure data length provided below expected HK header + trailer size! \n");
#endif
    }
    else
    {
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
                    OS_printf(
                        "  NOVATEL_OEM615_ReadHK: No DEAD header + BEEF trailer found when reading uart port! \n");
#endif
                }
            }
        }

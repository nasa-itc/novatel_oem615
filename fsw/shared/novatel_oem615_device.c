...
*/
int32_t NOVATEL_OEM615_CommandDevice(uart_info_t *uart_device, uint8_t cmd_code,
                                     uint32_t payload) {
  ...
  if (status == UART_SUCCESS) {
    // Write data
    bytes = uart_write_port(uart_device, write_data,
                            NOVATEL_OEM615_DEVICE_CMD_SIZE);
    ...
    if (bytes != NOVATEL_OEM615_DEVICE_CMD_SIZE) {
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("NOVATEL_OEM615_CommandDevice - uart_write_port returned %d, "
                "expected %d \n",
      ...
      if (bytes_available > 0) {
        bytes = uart_read_port(uart_device, read_data,
                               NOVATEL_OEM615_DEVICE_CMD_SIZE);
        if (bytes != NOVATEL_OEM615_DEVICE_CMD_SIZE) {
#ifdef NOVATEL_OEM615_CFG_DEBUG
          OS_printf("NOVATEL_OEM615_CommandDevice - uart_read_port returned "
                    "%d, expected %d \n",
                    bytes, NOVATEL_OEM615_DEVICE_CMD_SIZE);
#endif
          status = OS_ERROR;
          ...
          if (status == OS_SUCCESS) {
            ...
            bytes = 0;
            while ((bytes < (int32_t)NOVATEL_OEM615_DEVICE_CMD_SIZE) &&
                   (status == OS_SUCCESS)) {
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf("NOVATEL_OEM615_CommandDevice - uart_bytes_available "
                  "returned no bytes available!");
#endif
        status = OS_ERROR;
      }
    }
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
    OS_printf("NOVATEL_OEM615_CommandDevice - uart_flush returned error with "
              "status = %d! No uart_write_port performed.",
              status);
#endif
    status = OS_ERROR;
  }
...
int32_t NOVATEL_OEM615_CommandDeviceCustom(uart_info_t *uart_device,
                                           uint8_t cmd_code, int8_t log_type,
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("NOVATEL_OEM615_CommandDeviceCustom - uart_flush returned "
                "error with status = %d! No uart_write_port performed.",
                status);
#endif
      status = OS_ERROR;
    }
...
*/
int32_t NOVATEL_OEM615_RequestHK(uart_info_t *uart_device,
                                 NOVATEL_OEM615_Device_HK_tlm_t *data) {
  int32_t status = OS_SUCCESS;
  uint8_t read_data[NOVATEL_OEM615_DEVICE_HK_SIZE] = {0};

  /* Command device to send HK */
  status = NOVATEL_OEM615_CommandDevice(uart_device,
                                        NOVATEL_OEM615_DEVICE_REQ_HK_CMD, 0);
  if (status == OS_SUCCESS) {
    /* Read HK data */
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf("  NOVATEL_OEM615_RequestHK: NOVATEL_OEM615_ReadHK returned "
                  "data with either invalid header [0x%02x%02x] or invalid "
                  "trailer [0x%02x%02x]!\n",
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("  NOVATEL_OEM615_RequestHK: Invalid data read from "
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
    OS_printf("  NOVATEL_OEM615_RequestHK: NOVATEL_OEM615_CommandDevice "
              "reported error %d \n",
              status);
#endif
  }
...
** Read HK from device
*/
int32_t NOVATEL_OEM615_ReadHK(uart_info_t *uart_device, uint8_t *read_data,
                              uint8_t data_length) {
  ...
  if (data_length < 4) {
#ifdef NOVATEL_OEM615_CFG_DEBUG
    OS_printf("  NOVATEL_OEM615_ReadHK: Structure data length provided below "
              "expected HK header + trailer size! \n");
#endif
  } else {
    ...
    if (bytes_available > 0) {
      uint8_t *temp_read_data =
          (uint8_t *)calloc(bytes_available, sizeof(uint8_t));
      ...
      if (bytes != bytes_available) {
        ...
        for (int i = 0; i < (bytes - data_length); i++) {
          if ((temp_read_data[i] == NOVATEL_OEM615_DEVICE_HDR_0) &&
              ...
              (temp_read_data[i + data_length - 2] ==
               NOVATEL_OEM615_DEVICE_TRAILER_0) &&
              (temp_read_data[i + data_length - 1] ==
               NOVATEL_OEM615_DEVICE_TRAILER_1)) {
        ...
        if (status != OS_SUCCESS) {
          free(temp_read_data);
#ifdef NOVATEL_OEM615_CFG_DEBUG
          OS_printf("  NOVATEL_OEM615_ReadHK: No DEAD header + BEEF trailer "
                    "found when reading uart port! \n");
#endif
        }
      }
    } else {
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("  NOVATEL_OEM615_ReadHK: No data available when attempting to "
                "read from uart port! \n");
#endif
    }
  }
...
*/
int32_t NOVATEL_OEM615_RequestData(uart_info_t *uart_device,
  ...
  status = NOVATEL_OEM615_CommandDevice(uart_device,
                                        NOVATEL_OEM615_DEVICE_REQ_DATA_CMD, 0);
  if (status == OS_SUCCESS) {
    /* check how many bytes are waiting on the uart */
    bytes_available = uart_bytes_available(uart_device);
    if (bytes_available > 0) {
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf(
            "  NOVATEL_OEM615_RequestData: Bytes read != to requested! \n");
#endif
        status = OS_ERROR;
        free(temp_read_data);
      } else {
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
          OS_printf("  NOVATEL_OEM615_RequestData: No #BESTXYZA token found "
                    "when reading uart port! \n");
#endif
          status = OS_ERROR;
          free(temp_read_data);
        }
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("  NOVATEL_OEM615_RequestData: No data available when "
                "attempting to read from uart port! \n");
#endif
      status = OS_ERROR;
    }
  } else {
#ifdef NOVATEL_OEM615_CFG_DEBUG
    OS_printf("  NOVATEL_OEM615_RequestData: CommandDevice returned error with "
              "status = %d! \n",
              status);
#endif
    status = OS_ERROR;
...
*/
int32_t
NOVATEL_OEM615_ChildProcessReadData(uart_info_t *uart_device,
                                    NOVATEL_OEM615_Device_Data_tlm_t *data) {
  int32_t status = OS_SUCCESS;
  int32_t bytes = 0;
  int32_t bytes_available = 0;
  ...
  if (bytes_available > 0) {
    ...
    if (bytes != bytes_available) {
#ifdef NOVATEL_OEM615_CFG_DEBUG
      OS_printf("  NOVATEL_OEM615_ChildProcessReadData: Bytes read != to "
                "requested! \n");
#endif
      status = OS_ERROR;
      free(temp_read_data);
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
        OS_printf("  NOVATEL_OEM615_ChildProcessReadData: No #BESTXYZA token "
                  "found when reading uart port! \n");
#endif
        status = OS_ERROR;
        free(temp_read_data);
...
#ifdef NOVATEL_OEM615_CFG_DEBUG
    OS_printf("  NOVATEL_OEM615_ChildProcessReadData: No data available when "
              "attempting to read from uart port! \n");
#endif
    status = OS_ERROR;
  }
...
*************************************************************************/
// Reference:  Section 1.1.1, p. 24, OEM6 Family Firmware Reference Manual,
// OM-20000129, Rev 8, January 2015 (file om-20000129.pdf) Reference:
// Section 3.2.17, pp. 420-422, OEM6 Family Firmware Reference Manual,
// OM-20000129, Rev 8, January 2015 (file om-20000129.pdf)
void NOVATEL_OEM615_ParseBestXYZA(
    NOVATEL_OEM615_Device_Data_tlm_t *device_data_struct) {
  ...
  if (token != NULL) {
    ...
    device_data_struct->SecondsIntoWeek =
        (uint32_t)device_data_struct->Fractions;

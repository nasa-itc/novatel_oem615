...
NOVATEL_OEM615_Device_HK_tlm_t
    Novatel_oem615HK; /* NOVATEL_OEM615 Housekeeping Telemetry Packet */
...
int process_command(int cc, int num_tokens,
  ...
  case CMD_NOVATEL_OEM615:
    if (check_number_arguments(num_tokens, 0) == OS_SUCCESS) {
      status =
          NOVATEL_OEM615_RequestData(&Novatel_oem615Uart, &Novatel_oem615Data);
  ...
  case CMD_CFG:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      ...
      status =
          NOVATEL_OEM615_CommandDeviceCustom(&Novatel_oem615Uart, 7, config, 0);
  ...
  case CMD_LOG:
    if (check_number_arguments(num_tokens, 2) == OS_SUCCESS) {
      ...
      status = NOVATEL_OEM615_CommandDeviceCustom(&Novatel_oem615Uart, 4,
                                                  log_type, period);
      if (status == OS_SUCCESS) {
  ...
  case CMD_UNLOG:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      log_type = atoi(tokens[0]);
      status = NOVATEL_OEM615_CommandDeviceCustom(&Novatel_oem615Uart, 5,
                                                  log_type, 0);
      if (status == OS_SUCCESS) {
